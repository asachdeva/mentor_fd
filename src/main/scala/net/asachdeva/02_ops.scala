...
object input_stream {
  ...
  final case class IStream(createInputStream: () => InputStream) { self =>
    ...
    def orElse(that: => IStream): IStream = 
      try {
        IStream(self.createInputStream)
      } catch {
        case e: Exception => IStream.empty
      }
  ...
  object IStream {
    ...
    /** Creates an empty stream.
      */
    val empty: IStream =
      IStream(() => new java.io.ByteArrayInputStream(Array.ofDim[Byte](0)))
...
object email_filter {
  ...
  final case class EmailFilter(matches: Email => Boolean) { self =>
    ...
    def &&(that: EmailFilter): EmailFilter =
      //EmailFilter((self.matches(_: Email) && that.matches(_: Email)))
  ...
  object EmailFilter {
    def senderIs(address: Address): EmailFilter = EmailFilter(
      _.sender == address
    )

    def recipientIs(address: Address): EmailFilter = EmailFilter(
      _.to.contains(address)
    )

    def subjectContains(phrase: String): EmailFilter = EmailFilter(
      _.subject.contains(phrase)
    )

    def bodyContains(phrase: String): EmailFilter = EmailFilter(
      _.body.contains(phrase)
    )
  }
  ...
  /** EXERCISE 4
    ...
    */
  lazy val emailFilter1: EmailFilter = EmailFilter.subjectContains("discount") && EmailFilter.bodyContains("N95") && EmailFilter.recipientIs(Address("jogn@doe.com")).unary_!
...
object contact_processing {
  final case class SchemaCSV(columnNames: List[String]) {
    def relocate(i: Int, j: Int): Option[SchemaCSV] =
      if (i < columnNames.length && j < columnNames.length)
        Some(
          copy(columnNames =
            columnNames.updated(i, columnNames(j)).updated(j, columnNames(i))
          )
        )
      else None
    ...
    def add(name: String): SchemaCSV =
      copy(columnNames = columnNames ++ List(name))
  }

  final case class ContactsCSV(
      schema: SchemaCSV,
    ...
    def rename(oldColumn: String, newColumn: String): ContactsCSV = {
      ...
      if (index < 0) self
        ...
        copy(schema = SchemaCSV(schema.columnNames.updated(index, newColumn)))
    }
          ...
          schema
            ...
            .map(schema =>
              copy(
                schema = schema,
                content =
                  content.map(row => row.updated(j, row(i)).updated(i, row(j)))
              )
            )
        else None
      }
  ...
  sealed trait MappingResult[+A] { self =>
    ...
    def flatMap[B](f: A => MappingResult[B]): MappingResult[B] =
      self match {
        case Success(warnings, value) =>
          f(value) match {
            case Success(warnings2, value) =>
              Success(warnings ++ warnings2, value)
    ...
    def zipWith[B, C](that: MappingResult[B])(
        f: (A, B) => C
    ): MappingResult[C] = (self zip that).map(f.tupled)
  }
  object MappingResult {
    final case class Success[+A](warnings: List[String], value: A)
        extends MappingResult[A]
    final case class Failure(errors: List[String])
        extends MappingResult[Nothing]
  ...
  final case class SchemaMapping(
      map: ContactsCSV => MappingResult[ContactsCSV]
  ) { self =>
    import MappingResult.{Success, Failure}
    /** EXERCISE 1
      *
      ...
      */
    def +(that: SchemaMapping): SchemaMapping = SchemaMapping(contactCSV => self.map(contactCSV) match {
      case Success(warnings1, first) => that.map(contactCSV) match {
        case Success(warnings2, second) => Success(warnings1 + warnings2, first + second)
        case Failure(errors) => Failure(errors)
      }
      case Failure(errors) => Failure(errors)
    })
    ...
    def orElse(that: SchemaMapping): SchemaMapping = SchemaMapping(
      contacts => self.map(contacts) match {
        case Success(warnings, value) => Success(warnings, value)
        case Failure(errors) => that.map(contacts) match {
          case Success(warnings, value) => Success(warnings, value)
          case Failure(errors) => Failure(errors)
        }
    })
    ...
    /** BONUS: EXERCISE 3
      ...
      */
    def protect(columnNames: Set[String]): SchemaMapping = SchemaMapping(csv => copy(csv.schema.columnNames = columnNames))
  }
  object SchemaMapping {
    ...
    def rename(oldName: String, newName: String): SchemaMapping = SchemaMapping { csv =>
      ...
      MappingResult.Success(if (csv == csv2) List("Eror message") else Nil, csv2)
    }
    ...
    ): SchemaMapping = SchemaMapping { csv =>
      // combine returns
      val csv2: Option[ContactsCSV] = csv.combine(leftColumn, rightColumn)(f(leftColumn, rightColumn))
      MappingResult.Success(if (csv == csv2.get) List("Couldn't combine columns") else Nil, csv2.get)
    }
    ...
    /** EXERCISE 6
      ...
      */
    def relocate(column: String, j: Int): SchemaMapping = SchemaMapping { csv =>
      val csv2: Option[ContactsCSV] = csv.relocate(column, j)
      MappingResult.Success(if (csv == csv2.get) List("Couldn't relocate columns") else Nil, csv2.get)
    }
    ...
    /** EXERCISE 7
      ...
      * specified name.
      */
    def delete(name: String): SchemaMapping = SchemaMapping { csv =>
      val csv2 = csv.delete(name)
      MappingResult.Success(if (csv == csv2) List("Couldn't relocate columns") else Nil, csv2)
    }
  }
...
object ui_events {
  ...
  final case class Listener(onEvent: GameEvent => Unit) { self =>
    ...
    /** EXERCISE 1
      ...
      */
    def +(that: Listener): Listener = Listener(event => self.onEvent(event) + that.onEvent(event))
    ...
    /** EXERCISE 2
      ...
      */
    def orElse(that: Listener): Listener = try {
      Listener(self.onEvent + that.onEvent)
    } catch{
      case e: Exception => Listener(self.onEvent + that.onEvent)
    }
  ...
  /** EXERCISE 5
    ...
    */
  lazy val solution = gfxUpdateListener.runOn(uiExecutionContext).debug +
    twinkleAnimationListener.debug + motionDetectionListener.debug
...
object education {
  ...
  object Question {
    final case class Text(question: String, checker: Checker[String])
  ...
  final case class QuizResult(
    ...
    def toBonus: QuizResult =
      QuizResult(0, bonusPoints + correctPoints, 0, Vector.empty)
      ...
      scala.util
        .Try {
          ...
          checker.isCorrect(submittedAnswer) match {
            case Left(string) =>
              QuizResult(0, 0, checker.points, Vector(string))
        ...
        .getOrElse(
          QuizResult(
            ...
            0,
            checker.points,
            Vector("The format of your answer was not recognized")
          )
        )
      ...
      Quiz { () =>
        ...
        question match {
          ...
          case MultipleChoice(question, choices, checker) =>
            val choicePrintout = choices.zipWithIndex
              .map { case (c, i) => s"${i}. ${c}" }
              .mkString("\n")
          ...
          case TrueFalse(question, checker) =>
  ...
  final case class Checker[-A](
      points: Int,
      isCorrect: A => Either[String, Unit]
  )
  object Checker {
    ...
    def isFalse(points: Int): Checker[Boolean] =
      Checker(
        points,
        v => if (!v) Right(()) else Left("The correct answer is false")
      )
    ...
    def isMultipleChoice(points: Int)(choiceNumber: Int): Checker[Int] =
      Checker(
        points,
        v =>
          if (v == choiceNumber) Right(())
          else Left(s"The correct answer is ${choiceNumber}")
      )
    ...
    def isText(points: Int)(text: String): Checker[String] =
      Checker(
        points,
        v =>
          if (v == text) Right(()) else Left(s"The correct answer is ${text}")
      )
  }
  ...
  lazy val exampleQuiz: Quiz =
    Quiz(
      Question.TrueFalse(
        "Is coffee the best hot beverage on planet earth?",
        Checker.isTrue(10)
      )
    )
}