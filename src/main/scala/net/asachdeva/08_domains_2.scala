...
object loyalty_program {
  ...
  final case class Portfolio(holdings: Map[LoyaltyCurrency, LoyaltyAmount]) {
    def +(that: (LoyaltyCurrency, LoyaltyAmount)): Portfolio =
      copy(holdings =
        holdings.updated(
          that._1,
          holdings.get(that._1).map(_ + that._2).getOrElse(that._2)
        )
      )
  }
  ...
  sealed trait RuleCalculation[+A] { self =>
    ...
    /** EXERCISE 4
      ...
      */
    def &&(
        that: RuleCalculation[Boolean]
    )(implicit ev: A <:< Boolean): RuleCalculation[Boolean] = {
    ...
    def as[B](implicit ev: A <:< B): RuleCalculation[B] =
      RuleCalculation.Widen(self)(ev)
  }
  object RuleCalculation {
    final case class Widen[A, B](rc: RuleCalculation[A])(implicit
        val ev: A <:< B
    ) extends RuleCalculation[B]
  ...
  object SystemAction {
    ...
    final case class ChangeStatus(status: UserProgramStatus)
        extends SystemAction
  }

  final case class Activity(
      action: Either[UserAction, SystemAction],
      instant: Instant
  )
...
object calendar {
  final case class HourOfDay(value: Int) {
    def to(that: HourOfDay): Stream[HourOfDay] =
      (value to that.value).toStream.map(HourOfDay(_))

    def until(that: HourOfDay): Stream[HourOfDay] =
      (value until that.value).toStream.map(HourOfDay(_))
  }
  object HourOfDay {
    val min = HourOfDay(0)
  ...
  object CalendarAppointment {
    val empty: CalendarAppointment =
      CalendarAppointment(HourOfDay(0), HourOfDay(0))
    val full: CalendarAppointment =
      CalendarAppointment(HourOfDay(0), HourOfDay(24))
  }
  ...
  def findFreeTimes(
      lengthInHours: Int,
      friends: Map[Person, MonthlySchedule]
  ): MonthlySchedule = ???
}
...
object cms {
  ...
  object Component {
    final case class Leaf[Context, State](render0: (Context, State) => Html)
        extends Component[Context, State] {
      def render(context: Context, state: State): Html = render0(context, state)
    }

    def make[Context, State](
        render: (Context, State) => Html
    ): Component[Context, State] = Leaf(render)
  ...
  sealed trait JsonPath { self =>
    def +(that: JsonPath): JsonPath =
      that match {
        ...
        case JsonPath.Index(parent, index) =>
          JsonPath.Index(self + parent, index)
      }
...
object data_processing {
  ...
  object Schema {
    ...
    final case class Described(description: String, schema: Schema)
        extends Schema
    final case class HasField(name: String, value: () => Schema) extends Schema
    final case class Intersect(left: () => Schema, right: () => Schema)
        extends Schema
    final case class Union(left: () => Schema, right: () => Schema)
        extends Schema
    final case class Sequence(elementSchema: () => Schema) extends Schema

    def hasField(name: String, value: => Schema): Schema =
      HasField(name, () => value)
  ...
  object Data {
    ...
    final case class Number(value: BigDecimal)
        extends AbstractData(Schema.Number)
    final case class Str(value: String) extends AbstractData(Schema.Str)
    final case class Date(value: java.time.LocalDate)
        extends AbstractData(Schema.Date)
    final case class DateTime(value: java.time.LocalDateTime)
        extends AbstractData(Schema.DateTime)
    final case class Record(value: Map[String, Data]) extends Data {
      def schema =
        value
          .map { case (name, data) => Schema.hasField(name, data.schema) }
