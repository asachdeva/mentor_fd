...
object recipes {
  ...
  sealed trait Recipe[+A] { self =>
    ...
    def bake(temp: Int, time: Int): Recipe[Baked[A]] =
      Recipe.Bake(self, temp, time)
  }
  object Recipe {
    case object Disaster extends Recipe[Nothing]
    final case class AddIngredient(ingredient: Ingredient)
        extends Recipe[Ingredient]
    final case class Bake[A](recipe: Recipe[A], temp: Int, time: Int)
        extends Recipe[Baked[A]]

    def addIngredient(ingredient: Ingredient): Recipe[Ingredient] =
  ...
  def make[A](recipe: Recipe[A]): A =
    recipe match {
      ...
      case AddIngredient(ingredient) =>
        println(s"Adding ${ingredient}"); ingredient
